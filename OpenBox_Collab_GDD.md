...
### 7.4 宠物属性
- 宠物固定有攻击百分比、防御百分比、生命百分比3项属性，这3项属性都加给主角，对宠物自身无影响。
- 这3个属性随宠物升级增长。
- 玩家在自己的角色面板看到的都是加总了这些属性后的结果。


### 7.5 宠物天赋技能

每个宠物拥有最多 `4` 个被动技能词条。根据宠物品质决定属性的初始数量。见表：Pets ->TalentSkillNum
- 天赋技能词条全部为加属性效果，并且按照一定权重随机生成。见表：Pets ->TalentSkill
- 天赋技能随着吞噬同名宠物可以升级，每吞噬1个同名宠物，已有的随机1个天赋技能升1级。升级后属性会增长。见表：Pets ->TalentSkill
- 玩家在自己的角色面板看到的都是加总了这些属性后的结果。
- 宠物吞噬等级：在宠物界面上表现为宠物下方的星数。一共5颗星，每吞噬1次涨1颗星，涨满5颗后，再换一种其他颜色的星从第1个位置开始覆盖之前的星。（未覆盖的保留）

### 7.6 宠物升级

- 消耗饲料，可升级宠物。
宠物最大等级100级。
实际见表Pets.xlsx -> PetLevelup分页。
- 在界面中，选中的宠物如果升下一级的饲料足够，则升级按钮可点，否则不可点。

### 7.7 宠物系统需要补充或明确的规则

以下规则建议在实现前确认，避免宠物背包、吞噬、升级、战斗结算之间出现歧义。

#### 7.7.1 宠物唯一实例与保存规则

- 每次抽到宠物都应生成一个独立宠物实例，而不是只记录宠物种类。
- 宠物实例至少需要保存：实例ID、宠物ID、等级、当前经验或饲料进度、吞噬次数、天赋词条列表、每个天赋词条等级、是否上阵。
- 玩家最多保存 `30` 个宠物实例。宠物列表界面显示当前数量，例如参考图中的 `25/30`。
- 背包满时继续抽宠物的处理需要明确：
  - [建议] 背包满时仍允许抽，但抽到后必须选择吞噬或替换已有宠物，不能直接保留到背包。
    这里应该是在未来做抽卡界面时，如果背包满了，那边有个提示不让继续抽，需要先回去分解。
  - [待讨论] 是否允许玩家手动分解、放生或删除未上阵宠物。
    需要增加一个手动的分解功能。分解后获得饲料。这里需要更新界面示意图，我就不更新了。
...
- [待讨论] 抽到宠物后选择“保留”时，是加入背包、替换当前选中宠物，还是替换同名宠物中的某一个实例。
    这里是直接进入背包。

#### 7.7.3 抽宠与初始生成规则
    整个抽宠物界面我们先不具体设计，先把宠物自己这块捋清楚。

- 抽宠需要明确消耗数量、抽取入口、可抽宠物池和各宠物权重。
- 抽到宠物时，根据宠物品质从 `TalentSkillNum` 读取初始天赋词条数量。
- 初始天赋词条从 `TalentSkill` 中随机生成。
- 同一只宠物初始生成时是否允许重复天赋词条需要明确。
  - [建议] 同一只宠物的天赋词条不重复，方便玩家理解和展示。
  不能重复。同类词条不同品质也不允许。
- 天赋词条品质的随机规则需要明确：先随机词条属性再随机词条品质，或先随机品质再随机词条属性。
  先随机词条属性
- [待讨论] 宠物自身品质是否等同于天赋词条品质，还是二者独立随机。
  二者独立随机

#### 7.7.4 吞噬规则

- 参考图表现为“只可吞噬未上阵同名灵兽”，建议规则明确为：只能吞噬同宠物ID、未上阵、非当前目标宠物的宠物实例。
    好的，按你说的办。
- 每吞噬 `1` 只同名宠物，目标宠物吞噬次数 +1，并在已有天赋词条中随机选择 `1` 条提升 `1` 级。
- 如果随机到的天赋词条已达到最高等级，需要明确处理：
  - [建议] 重新随机未满级词条。
    这里的随机规则是：在随机的时候，首先排除掉已升满的词条。
  - [待讨论] 若所有词条都满级，是否禁止吞噬或只增加星级表现。若
    词条满级，则星级也应该满了，所以就不能再吞噬了。
- 吞噬素材宠物被消耗后从背包中移除。若被吞噬的宠物等级大于1，则同时会返还饲料资源，按分解的比例。
- 吞噬不改变宠物主技能。
- 吞噬次数上限从 `TalentSkillNum.AbsorbLimit` 读取。当前表格逻辑为：`天赋技能数量 * (天赋技能最高等级 - 1)`。
- 星级表现只用于展示吞噬进度，不等同于宠物品质。星色循环规则需要明确。
    这里你要理解：宠物品质和星级没有任何关系。
  - [建议] 每 `5` 次吞噬换一种星色，实际吞噬次数仍按数值保存。
    按你说的办。

#### 7.7.5 天赋属性生效规则

- 宠物固定属性和天赋属性都加给主角，并计入角色面板。
- 上阵宠物的属性生效；未上阵宠物不生效。
- 百分比属性沿用全局规则：表格填写 `5` 表示 `5%`。
- `TalentSkill` 中的 `Rarity1Value/Rarity1Delta` 等字段需要明确公式：
  - [建议] 天赋最终数值 = 初始值 + 成长值 * (天赋等级 - 1)。
    没问题。
- [待讨论] 宠物固定属性、天赋属性、装备属性之间是直接相加，还是先合并基础属性再参与战斗公式。
    所有百分比类的属性都是全部相加后，再与基础值相乘。

#### 7.7.6 宠物主技能战斗规则

- 主技能由 `Pets` 表配置，战斗中自动触发。
- 需要明确宠物技能计时单位：
  - [建议] `CastInterval` 表示己方角色行动回合数，每当我方角色完成一次行动后计数 +1，达到间隔后释放宠物技能。
    这里按这个：表示己方角色行动回合数，每当我方角色要行动前，计数 +1，达到间隔后释放宠物技能。也就是宠物的行动会在角色之前。
- 需要明确首次释放时机：
  - [建议] 战斗开始后从 0 计数，不在开场立即释放，达到 `CastInterval` 后首次释放。
  没问题。
- 攻击型宠物技能伤害建议使用：主角攻击力 * `DamagePercent`，并受“强化宠物/弱化宠物”和最终增减伤影响。
没问题。
- 治疗型宠物技能治疗建议使用：主角攻击力 * `HealPercent`，并受“强化治疗/弱化治疗”影响。
没问题。
- 宠物技能是否能触发暴击、吸血、击晕、连击、反击需要明确。
  - [建议] 宠物技能不触发普通攻击衍生效果，避免战斗链路过长。
  没问题。
- Buff 类技能需要明确叠加、刷新和回合扣减规则：
  - [建议] 同属性同来源 Buff 叠加到 `MaxStack`；再次获得时刷新持续回合；`DurationRounds=999` 视为持续到战斗结束。
  没问题。
...
- 宠物最高等级为 `100`。满级后升级按钮不可点击。
- [待讨论] 饲料来源需要明确：闯关奖励、吞噬返还、开箱获得，或其他系统产出。
这个后续再讨论，先留着
- [待讨论] 宠物升级是否 MVP 必做。当前 2.2 写“复杂宠物升级”可暂缓，但本章节已定义升级按钮和 100 级规则，需要统一版本范围。
必须做。把2.2那里改了。

#### 7.7.8 界面与操作反馈规则

- 宠物界面需要展示：当前上阵宠物、宠物品质、等级、吞噬星级、主技能、固定属性、天赋词条、宠物背包数量。
- 宠物吞噬界面需要展示：目标宠物、可选同名素材、不可选原因、预计吞噬收益。
- 吞噬前需要二次确认，尤其当素材宠物等级、品质或天赋较高时。
- 升级、上阵、吞噬后需要立即刷新角色面板属性和战力。
- [待讨论] 是否需要锁定宠物功能，防止误吞噬。
先不做，留着后续加。

#### 7.7.9 数据表补充建议

- `Pets` 页建议补充抽取权重、技能目标、技能释放时机、技能是否可暴击、技能特效资源字段。
技能目标只有敌我双方的角色。根据技能类型就可以区分。其他的目前用不到，后面再补。
- `PetLevelup` 页建议确认 100 级行是否表示“100级属性”还是“100升101消耗”。如果最高等级为 100，则 100 级不应存在升下一级消耗，或需要约定忽略。
约定忽略
- `TalentSkillNum` 页建议补充星色循环或显示等级配置，避免表现逻辑硬编码。
我新加了一页stars。到时候你顺便直接帮我生成星星的各种颜色的资源图片吧。
- `TalentSkill` 页当前包含品质4字段，但 `Pets` 当前只有品质1-3。需要确认是否预留品质4，还是首版暂不读取品质4。
保留，暂时不读。后面我会再加宠物。
- 建议新增抽宠权重页，例如 `PetGacha`：宠物ID、权重、保底规则、消耗宠物券数量。
等做抽宠物的时候再加。
- 建议新增资源配置页，例如 `PetResource`：饲料来源、宠物券来源、吞噬返还规则。
我在PetLevelup表中新加了FoodReturn列。
---
...
| Q015 | 排行榜是否需要后端？ | 低 | 待定 |
| Q016 | 背包满时抽到宠物，是禁止抽取、强制吞噬，还是允许替换已有宠物？ | 高 | 待定 |
| Q017 | 抽到宠物后选择“保留”，是加入背包、替换当前选中宠物，还是替换同名宠物实例？ | 高 | 待定 |
| Q018 | 同一只宠物的初始天赋词条是否允许重复？ | 中 | 待定 |
| Q019 | 天赋词条品质如何随机？宠物品质是否等同于天赋词条品质？ | 高 | 待定 |
| Q020 | 吞噬时随机到已满级天赋词条如何处理？ | 中 | 待定 |
| Q021 | 宠物技能释放间隔按己方行动、双方总回合，还是宠物自身回合计数？ | 高 | 待定 |
| Q022 | 宠物技能是否触发暴击、吸血、击晕、连击、反击等普通攻击衍生效果？ | 高 | 待定 |
| Q023 | 饲料来源是什么？宠物升级是否进入 MVP？ | 高 | 待定 |
| Q024 | 是否需要宠物锁定功能，防止误吞噬？ | 低 | 待定 |
